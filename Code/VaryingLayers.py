...
regres=Sequential()
regres.add(Dense(output_dim=3,kernel_initializer='normal',activation='relu',input_dim=Spec_dim))#Adding the input and hidden layer 1
for i in range(Spec_layers-1):
    regres.add(Dense(output_dim=6,kernel_initializer='normal'))
...
plt.plot(y1,'.-', color='purple')
plt.plot(y2,'.-', color='orange')
plt.xlabel('YEARS')
plt.ylabel('Rainfall Predicted(Purple) vs Actual(Orange)')
plt.title('Predicted Vs Test Results')
plt.show()
